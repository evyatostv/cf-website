  ...
  return (
    ...
    >
      <div className="flex items-start gap-4">
        <div className="flex-shrink-0 w-14 h-14 rounded-2xl bg-[#e8f4f8] flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
    ...
    <section id="features" className="py-32 bg-white relative overflow-hidden">
      <div className="container mx-auto px-6 max-w-7xl" ref={ref}>
        ...
        <p className="text-sm text-[#6b7c93] text-center mb-10 bg-[#f5f7f9] rounded-xl px-6 py-3 max-w-2xl mx-auto border border-[#e1e6ec]">
          * זמינות הפיצ'רים תלויה בחבילה הנרכשת. חלק מהיכולות זמינות בחבילות המקצועית או המלאה בלבד.{" "}
          <Link to="/pricing" className="text-[#0d47a1] font-medium hover:underline">ראו עמוד התמחור לפירוט מלא.</Link>
        </p>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-10">
          {features.map((feature, index) => (
            <Feature key={index} {...feature} delay={0} />
          ))}
        </div>
        ...
        >
          <div className="inline-flex flex-col sm:flex-row items-center gap-4 bg-white rounded-3xl p-8 border border-[#e1e6ec] shadow-lg">
            <div className="text-right">
              <h3 className="text-2xl font-semibold text-[#1a2332] mb-2">רוצים לראות איך זה עובד אצלכם?</h3>
              <p className="text-[#6b7c93]">נראה לכם את האפליקציה על מקרה אמיתי מהמרפאה שלכם</p>
            </div>
            <motion.button
